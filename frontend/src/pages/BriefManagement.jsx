...
export default function BriefManagement() {
  ...
  const handleSaveBrief = async (e) => {
    e.preventDefault();
    if (!activeSale || !briefForm.fileName || !briefForm.fileUrl) {
      return toast.error('File name and file content are required');
    }
    try {
      setUploading(true);
      await api.post(`/sales/${activeSale.id}/briefs`, briefForm);
      toast.success(`Brief "${briefForm.fileName}" uploaded & assigned!`);
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-extrabold tracking-tight text-white font-display uppercase flex items-center gap-2">
            ...
            Project Briefs & Artwork Asset Repository
          </h2>
          <p className="text-xs text-brand-text-soft mt-1">
            CEO uploads & assigns PNG, JPEG, WEBP & PDF briefs to Artists. Artists review briefs and post deliverable updates.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative min-w-[220px]">
            ...
            <input
              type="text"
              placeholder="Search project, brief, artist..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-9 pr-4 py-1.5 rounded-xl border border-brand-border bg-brand-bg-soft/40 text-xs text-white placeholder-brand-text-mute focus:outline-none focus:border-brand-blue"
            />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div className="col-span-full py-16 text-center text-brand-text-mute italic border border-dashed border-brand-border rounded-2xl">
            No project briefs found matching your criteria.
          </div>
        ) : (
              ...
              >
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between border-b border-brand-border/40 pb-2">
                    <span className="px-2.5 py-0.5 rounded bg-brand-blue/20 text-brand-cyan font-mono text-[10px] font-bold">
                      {sale.projectNumber}
                    </span>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-brand-text-soft">Client: {sale.clientName}</p>
                  </div>

                  {/* Assigned Artist Badge */}
                  <div className="flex items-center justify-between p-2.5 rounded-xl bg-brand-bg-elevated/40 border border-brand-border/30">
                    ...
                    {(isCEOOrAdmin || isTL) && (
                      <button
                        ...
                        className="px-2 py-1 rounded bg-brand-violet/15 hover:bg-brand-violet/30 text-brand-violet border border-brand-violet/30 text-[9px] font-bold uppercase flex items-center gap-1 transition-all cursor-pointer"
                      >
                  ...
                  {latestBrief ? (
                    <div className="p-3 rounded-xl bg-brand-bg/60 border border-brand-border/40 space-y-2">
                      <div className="flex justify-between items-center text-xs">
                        <div className="flex items-center gap-2 min-w-0">
                          {['png', 'jpeg', 'jpg', 'webp'].includes(latestBrief.fileType) ? (
                            <ImageIcon className="w-4 h-4 text-brand-cyan shrink-0" />
                          ) : (
                            <FileText className="w-4 h-4 text-brand-amber shrink-0" />
                          )}
                          <span className="font-bold text-white truncate text-xs">{latestBrief.fileName}</span>
                        </div>
                        <span className="px-2 py-0.5 rounded bg-brand-cyan/15 text-brand-cyan font-mono text-[9px] font-bold shrink-0">
                      ...
                      {['png', 'jpeg', 'jpg', 'webp'].includes(latestBrief.fileType) && latestBrief.fileUrl && (
                        <div className="relative rounded-lg overflow-hidden border border-brand-border/40 max-h-36 bg-black/40 group">
                          <img
                      ...
                      {latestBrief.notes && (
                        <p className="text-[10px] text-brand-text-soft italic">CEO Notes: "{latestBrief.notes}"</p>
                      )}

                      {/* Artist Update Note */}
                      {latestBrief.artistUpdate && (
                        <div className="p-2 rounded bg-brand-blue/10 border border-brand-blue/30 text-[10px]">
                          <p className="font-bold text-brand-cyan">Artist Progress Update:</p>
                          <p className="text-white italic">"{latestBrief.artistUpdate}"</p>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="p-4 rounded-xl bg-brand-bg/20 border border-dashed border-brand-border text-center">
                ...
                <div className="pt-3 border-t border-brand-border/40 space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    {(isCEOOrAdmin || isTL) && (
                      <button
                        ...
                        className="flex-1 py-1.5 px-3 rounded-xl bg-gradient-to-r from-brand-blue to-brand-cyan text-brand-bg font-bold text-[10px] uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer"
                      >
                    ...
                    {latestBrief && (
                      <a
                        ...
                        className="py-1.5 px-3 rounded-xl bg-brand-bg border border-brand-border text-brand-cyan font-bold text-[10px] uppercase flex items-center gap-1.5 hover:border-brand-cyan/50"
                      >
                  ...
                  {latestBrief && (isDesigner || isCEOOrAdmin || isTL) && (
                    <button
                      onClick={() => {
                      ...
                      className="w-full py-1.5 px-3 rounded-xl bg-brand-violet/15 hover:bg-brand-violet/25 text-brand-violet border border-brand-violet/30 font-bold text-[10px] uppercase flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                    >
                      <MessageSquare className="w-3.5 h-3.5" /> Post Artist Brief Update
                    </button>
                  )}
                </div>
              </motion.div>
      ...
      {/* CEO Brief Upload Modal */}
      <AnimatePresence>
        {uploadModalOpen && activeSale && (
            ...
            >
              <div className="flex items-center justify-between border-b border-brand-border pb-3">
                <div>
                  ...
                  <h3 className="text-sm font-extrabold text-white font-display">Upload Brief File</h3>
                </div>
              ...
              <form onSubmit={handleSaveBrief} className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-brand-text-soft uppercase mb-1">Select File (PNG, JPEG, WEBP, PDF, DOCX)</label>
                  <input
                    type="file"
                    accept={ACCEPTED_TYPES}
                    onChange={handleFileUpload}
                    required
                    className="w-full text-xs text-brand-text-soft file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-brand-blue/20 file:text-brand-cyan hover:file:bg-brand-blue/30 cursor-pointer"
                  />
                  <p className="text-[9px] text-brand-text-mute mt-1">Accepts PNG, JPEG, JPG, WEBP, PDF & DOCX</p>
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-brand-text-soft uppercase mb-1">Assign to Artist</label>
                  <select
                  ...
                  >
                    <option value="">Unassigned (Select Artist)</option>
                    {designers.map(d => (
                ...
                <div>
                  <label className="block text-[10px] font-bold text-brand-text-soft uppercase mb-1">Brief Notes / Requirements</label>
                  <textarea
                    rows={3}
        ...
        {assignModalOpen && activeSale && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-brand-bg-elevated border border-brand-violet/40 rounded-2xl p-6 max-w-sm w-full text-left space-y-4 shadow-glow"
            >
              <div className="flex items-center justify-between border-b border-brand-border pb-3">
                <div>
              ...
              <form onSubmit={handleAssignArtist} className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-brand-text-soft uppercase mb-1">Select Artist (Designer)</label>
                  <select
        ...
        {updateModalOpen && activeBrief && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            >
              <div className="flex items-center justify-between border-b border-brand-border pb-3">
                <div>
                  <h3 className="text-sm font-extrabold text-white font-display">Post Artist Brief Update</h3>
              ...
              <form onSubmit={handleArtistUpdate} className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-brand-text-soft uppercase mb-1">Brief Status</label>
                  <select
                ...
                <div>
                  <label className="block text-[10px] font-bold text-brand-text-soft uppercase mb-1">Artist Update / Deliverable Notes</label>
                  <textarea
                    rows={3}
                    value={artistUpdateData.artistUpdate}
                    onChange={e => setArtistUpdateData({ ...artistUpdateData, artistUpdate: e.target.value })}
                    placeholder="e.g. Completed initial sketch outline, working on base colors..."
                    className="w-full px-3.5 py-2.5 rounded-xl border border-brand-border bg-brand-bg text-xs text-white focus:outline-none"
                  />
                </div>
                ...
                <div className="flex gap-3 pt-2 border-t border-brand-border">
                  ...
                  <button
                    type="submit"
                    className="flex-1 py-2 rounded-xl bg-gradient-to-r from-brand-violet to-brand-blue text-white font-bold text-xs uppercase"
                  >
                    Post Update
                  </button>
                </div>
        ...
        {previewModalOpen && activeBrief && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            ...
            >
              ...
              <img src={activeBrief.fileUrl} alt={activeBrief.fileName} className="max-h-[80vh] mx-auto rounded-xl border border-brand-border shadow-glow object-contain" />
            </motion.div>
          </div>
        )}
      </AnimatePresence>
